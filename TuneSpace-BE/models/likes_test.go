...
func testLikesExists(t *testing.T) {
	...
	e, err := LikeExists(ctx, tx, o.UserID, o.TrackID)
	if err != nil {
...
func testLikesFind(t *testing.T) {
	...
	likeFound, err := FindLike(ctx, tx, o.UserID, o.TrackID)
	if err != nil {
		t.Error(err)
	}
...
func testLikeToOneTrackUsingTrack(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = local.L.LoadTrack(ctx, tx, false, (*[]*Like)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testLikeToOneSetOpTrackUsingTrack(t *testing.T) {
	var err error
	...
	for i, x := range []*Track{&b, &c} {
		...
		if exists, err := LikeExists(ctx, tx, a.UserID, a.TrackID); err != nil {
			t.Fatal(err)
		} else if !exists {
...
var (
	likeDBTypes = map[string]string{`UserID`: `bigint`, `TrackID`: `bigint`, `CreatedAt`: `timestamp with time zone`}
	_           = bytes.MinRead
)
