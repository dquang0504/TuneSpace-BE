...
func testCommentsInsertWhitelist(t *testing.T) {
	...
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	if err = o.Insert(ctx, tx, boil.Whitelist(strmangle.SetMerge(commentPrimaryKeyColumns, commentColumnsWithoutDefault)...)); err != nil {
		t.Error(err)
	}
...
func testCommentToOneTrackUsingTrack(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = local.L.LoadTrack(ctx, tx, false, (*[]*Comment)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testCommentToOneSetOpTrackUsingTrack(t *testing.T) {
	var err error
...
func testCommentToOneRemoveOpTrackUsingTrack(t *testing.T) {
	var err error
...
func testCommentToOneRemoveOpUserUsingUser(t *testing.T) {
	var err error
...
var (
	commentDBTypes = map[string]string{`ID`: `bigint`, `UserID`: `bigint`, `TrackID`: `bigint`, `Content`: `text`, `CreatedAt`: `timestamp with time zone`}
	_              = bytes.MinRead
)
