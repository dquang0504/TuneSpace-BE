...
func testAlbumsInsertWhitelist(t *testing.T) {
	...
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	if err = o.Insert(ctx, tx, boil.Whitelist(strmangle.SetMerge(albumPrimaryKeyColumns, albumColumnsWithoutDefault)...)); err != nil {
		t.Error(err)
	}
...
func testAlbumToManyArtistsAlbums(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := AlbumSlice{&a}
	if err = a.L.LoadArtistsAlbums(ctx, tx, false, (*[]*Album)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testAlbumToManyAddOpArtistsAlbums(t *testing.T) {
	var err error
...
var (
	albumDBTypes = map[string]string{`ID`: `bigint`, `Title`: `text`, `ArtistID`: `bigint`, `ReleaseDate`: `date`, `CoverURL`: `text`, `CreatedAt`: `timestamp with time zone`, `Status`: `boolean`}
	_            = bytes.MinRead
)
